...
def parse_salary_slip(file):
    """
    Advanced Multi-Block Parser with 7th CPC Pay Matrix Validation for Max Basic Pay.
    Ignores non-standard/arrear/combined block amounts from becoming the primary Basic Pay.
    """
    extracted = {
    ...
    # Standard 7th CPC Pay Matrix valid Basic Pay increments list to filter out arrear/combined anomalies
    VALID_7TH_CPC_BASIC = {
    ...
    try:
        ...
        for block in salary_blocks:
            ...
            if b_val > 0:
                ...
                # Check if this basic matches 7th CPC valid matrix list (filtering out arrear/combined blocks)
                if int(b_val) in VALID_7TH_CPC_BASIC:
        ...
        # Select maximum basic strictly from valid 7th CPC list, else fallback to highest available record
        if valid_cpc_basics:
            latest_basic = max(valid_cpc_basics)
        else:
            latest_basic = max([r['basic'] for r in monthly_records]) if monthly_records else 58600.0
        ...
        print(f"Error in 7th CPC validated parser: {e}")
