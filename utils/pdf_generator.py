...
def generate_form16_pdf(data, is_trial=False):
    ...
    and Monthly Salary Statement layout with A4 fit, proper tables, and conditional footer/watermark.
    """
    try:
        ...
        ptax = float(data.get('ptax', 200))
        gpf = float(data.get('gpf', 0))
        tds = float(data.get('tds', 0))
        gli = float(data.get('gli', 60))
        net_income = float(data.get('net_income', gross - (ptax + gpf + tds + gli)))
    except:
...
<div class="header-box" style="margin-top: 5px;">
  ...
  <p>I, <b>{{ data.ddo_officer }}</b> son/daughter of <b>{{ data.ddo_father }}</b> working in the capacity of <b>DRAWING AND DISBURSING OFFICER</b> do hereby certify that a sum of Rs. <b>{{ tds }}</b> has been deducted and deposited to the credit of the Central Government.</p>
...
<table>
  <tr>
    ...
    <td width="20%" class="right"><br>Rs. {{ gross }}<br>Rs. 0.00<br>Rs. 0.00<br>Rs. {{ gross }}<br>Rs. 0.00</td>
  </tr>
  <tr><td><b>2. LESS: ALLOWANCE TO THE EXTENT EXEMPT U/S 10</b></td><td class="right">Rs. 0.00</td></tr>
  <tr><td><b>3. BALANCE (1 - 2)</b></td><td class="right">Rs. {{ gross }}</td></tr>
  <tr>
    ...
    <td class="right"><br>Rs. 75,000.00<br>Rs. 0.00<br>Rs. {{ ptax }}</td>
  </tr>
  <tr><td><b>5. AGGREGATE OF DEDUCTIONS U/S 16</b></td><td class="right">Rs. {{ ptax + 75000 }}</td></tr>
  <tr><td><b>6. INCOME CHARGEABLE UNDER THE HEAD "SALARIES" (3 - 5)</b></td><td class="right">Rs. {{ gross - (ptax + 75000) }}</td></tr>
  <tr><td><b>7. ADD: ANY OTHER INCOME REPORTED BY EMPLOYEE</b></td><td class="right">Rs. 0.00</td></tr>
  <tr><td><b>8. GROSS TOTAL INCOME (6 + 7)</b></td><td class="right">Rs. {{ gross - (ptax + 75000) }}</td></tr>
</table>
...
<table>
  ...
  <tr><td>80C (GPF / LIC / etc.)</td><td class="right">{{ gpf }}</td><td class="right">-</td><td class="right">{{ gpf }}</td></tr>
  <tr><td>80CCD(1B) / 80D / 80G</td><td class="right">0.00</td><td class="right">-</td><td class="right">0.00</td></tr>
  <tr class="bold"><td>Total Deductions under Chapter VI-A</td><td colspan="3" class="right">{{ gpf }}</td></tr>
</table>

<table>
  <tr><td><b>10. AGGREGATE OF DEDUCTIBLE AMOUNT UNDER CHAPTER VI-A</b></td><td class="right" width="25%">Rs. {{ gpf }}</td></tr>
  <tr><td><b>11. TOTAL TAXABLE INCOME (8 - 10)</b></td><td class="right">Rs. {{ (gross - (ptax + 75000)) - gpf }}</td></tr>
  ...
  <tr><td><b>21. NET TAX PAYABLE / REFUNDABLE</b></td><td class="right">Rs. {{ tds }}</td></tr>
</table>
...
<table>
  <tr class="bold center" style="background-color: #f2f2f2;">
    ...
    <td>चिकित्सा</td>
    <td>कुल योग (आय)</td>
    <td>जी.पी.एफ. / सी.पी.एफ.</td>
    <td>जी.आई.एस.</td>
    <td>व्यावसायिक कर (PTax)</td>
    <td>कटौतियों का योग</td>
    <td>आयकर (TDS)</td>
    <td>शुद्ध वेतन (Net Pay)</td>
  </tr>
  {% set months = ["मार्च 25", "अप्रैल 25", "मई 25", "जून 25", "जुलाई 25", "अगस्त 25", "सितम्बर 25", "अक्टूबर 25", "नवम्बर 25", "दिसम्बर 25", "जनवरी 26", "फरवरी 26"] %}
  {% for m in months %}
  <tr>
    <td><b>{{ m }}</b></td>
    ...
    <td class="right">{{ basic + da + hra + medical }}</td>
    <td class="right">{{ gpf // 12 }}</td>
    <td class="right">60.00</td>
    <td class="right">{{ ptax }}</td>
    <td class="right">{{ (gpf // 12) + 60 + ptax }}</td>
    <td class="right">{{ tds // 12 }}</td>
    <td class="right">{{ (basic + da + hra + medical) - ((gpf // 12) + 60 + ptax + (tds // 12)) }}</td>
  </tr>
  {% endfor %}
  <tr class="bold" style="background-color: #e6e6e6;">
    <td><b>योग (TOTAL)</b></td>
    <td class="right">{{ basic * 12 }}</td>
    <td class="right">{{ da * 12 }}</td>
    <td class="right">{{ hra * 12 }}</td>
    <td class="right">{{ medical * 12 }}</td>
    <td class="right">{{ gross }}</td>
    <td class="right">{{ gpf }}</td>
    <td class="right">720.00</td>
    <td class="right">{{ ptax * 12 }}</td>
    <td class="right">{{ gpf + 720 + (ptax * 12) }}</td>
    <td class="right">{{ tds }}</td>
    <td class="right">{{ net_income }}</td>
  </tr>
</table>
