...
def parse_slip_in_memory(uploaded_file):
    ...
    try:
        ...
        if pan_m:
            extracted['pan'] = pan_m.group(1).strip()
        ...
        if des_m:
            extracted['designation'] = des_m.group(1).strip()

        # 4. GPF Number
        gpf_m = re.search(r'Employee\s*GPF\s*No\.?\s*[:\-]?\s*([A-Z0-9\/\-]+)', raw_clean, re.I)
        if gpf_m:
            extracted['gpf_no'] = gpf_m.group(1).strip()

        # 5. Bank Account Number
        ac_m = re.search(r'Employee\s*A\/C\s*No\.?\s*[:\-]?\s*([0-9]+)', raw_clean, re.I)
        if ac_m:
            extracted['account_no'] = ac_m.group(1).strip()

        # 6. DDO Code
        ddo_m = re.search(r'DDO\s*CODE\s*[:\-]?\s*([A-Z0-9]+)', raw_clean, re.I)
        if ddo_m:
            extracted['ddo_code'] = ddo_m.group(1).strip()

        # Helper: Extract Amount
        def get_amount(labels):
            for label in labels:
                pat = rf"{re.escape(label)}\s*[:\-]?\s*[₹Rs\.\s]*([0-9][0-9,]*(?:\.[0-9]+)?)"
                m = re.search(pat, raw_clean, flags=re.I)
                if m:
                    try:
        ...
        # Allowances
        extracted['basic'] = get_amount(["Basic", "मूल वेतन"])
        extracted['da'] = get_amount(["DA", "Dearness Allowance", "महंगाई"])
        extracted['hra'] = get_amount(["HRA", "House Rent Allowance"])
        med_val = get_amount(["Medical Allow.", "Medical Allowance", "चिकित्सा"])
        if med_val > 0:
            extracted['medical'] = med_val

        # Deductions
        extracted['gpf'] = get_amount(["GPF", "NPS", "PRAN"])
        gis_val = get_amount(["GLI", "GIS"])
        if gis_val > 0:
            extracted['gis'] = gis_val
        ptax_val = get_amount(["PROF TAX", "Professional Tax", "P.TAX"])
        if ptax_val > 0:
            extracted['ptax'] = ptax_val
        extracted['tds'] = get_amount(["I.TAX", "ITAX", "I. TAX", "1.TAX", "1. TAX", "TDS"])

    except Exception:
        pass

    if extracted['basic'] > 0:
        if extracted['da'] == 0:
            extracted['da'] = round(extracted['basic'] * 0.53)
        if extracted['hra'] == 0:
            extracted['hra'] = round(extracted['basic'] * 0.09)
        if extracted['gpf'] == 0:
            extracted['gpf'] = round(extracted['basic'] * 0.10)
    
    return extracted

# ================= DUAL TAX ENGINE (NEW & OLD REGIME) WITH FEB BALANCING =================
def compute_annual_tax_with_regime(records_11, regime="New Regime", decl_data=None, manual_arrear_da=0.0, manual_arrear_pay=0.0):
...
# ================= 4-PAGE BILINGUAL PRINT TEMPLATE =================
...
<div class="sheet-page">
  ...
  <table class="border">
    ...
    <tr class="bold bg-gray"><td>12.</td><td>वित्तीय वर्ष 2025-26 में भुगतेय आयकर / (रिफंड)</td><td class="right">{{ "%.2f"|format(tax.net_balance) }}</td></tr>
  </table>
...
# ================= SHARED GENERATOR SUITE WITH AUTO-FILL =================
def render_full_employee_suite(is_admin_mode=False, prefix="emp"):
    st.markdown("#### 📄 Salary Slip Upload & Instant Auto-Fill")
    slip_up = st.file_uploader(
        "Upload Salary Slip (PDF) — PAN, Name, Basic, DA, HRA & Deductions auto-fill ho jayenge[span_2](start_span)[span_2](end_span)[span_3](start_span)[span_3](end_span):", 
        type=["pdf"], 
        key=f"{prefix}_slip"
    )
    ...
    if scanned and scanned.get('pan'):
        st.success(f"⚡ **Auto-Filled from Slip:** Name: **{scanned['name']}** | PAN: **{scanned['pan']}** | Basic: **₹{scanned['basic']:,.0f}** | DA: **₹{scanned['da']:,.0f}** | HRA: **₹{scanned['hra']:,.0f}**[span_4](start_span)[span_4](end_span)[span_5](start_span)[span_5](end_span)")

    # 1. PAN Input
    scanned_pan = scanned['pan'] if scanned and scanned.get('pan') else ""
    ...
    def_st_idx = states_list.index(prof[7]) if prof and prof[7] in states_list else 0
    with c_l1:
    ...
    def_dt_idx = dist_list.index(prof[8]) if prof and prof[8] in dist_list else 0
    with c_l2:
    ...
    def_dp_idx = dept_list.index(prof[9]) if prof and prof[9] in dept_list else 0
    with c_l3:
    ...
    with c_d2:
        ...
        def_o_idx = off_list.index(prof[10]) if prof and prof[10] in off_list else 0
    ...
    # Pay Matrix & Increment calculations
    c_m1, c_m2, c_m3, c_m4 = st.columns(4)
    h_level = prof[12] if prof else "Level 7"
    h_basic = prof[13] if prof else 49000.0
    h_inc_m = prof[14] if prof else "1st JULY"
    h_pen = prof[15] if prof else "NPS"
    h_vgpf = prof[16] if prof else 0.0

    if scanned and scanned.get('basic', 0) > 0: 
        h_basic = scanned['basic']
    if prof and status_sel == "NORMAL (Regular Continuity)": 
        h_basic = get_next_matrix_cell(h_level, h_basic)
    ...
    with c_pen1: s_pen = st.radio("Pension Scheme", ["NPS (10% Basic+DA)", "Old Pension (GPF)"], index=0 if h_pen == "NPS" else 1, horizontal=True, key=f"{prefix}_pen")
    with c_pen2: s_vgp = st.number_input("Voluntary GPF/Mo", value=int(h_vgpf), step=500, key=f"{prefix}_vgp") if "GPF" in s_pen else 0.0
    ...
    with st.container(border=True):
        ...
        with c_v1:
            val_name = scanned['name'] if scanned and scanned.get('name') else (prof[1] if prof else "Employee Name")
            val_des = scanned['designation'] if scanned and scanned.get('designation') else (prof[2] if prof else "+2 Teacher")
            v_name = st.text_input("Full Name *", value=val_name, key=f"{prefix}_vn")
            v_des = st.text_input("Designation", value=val_des, key=f"{prefix}_vd")
        with c_v2:
        ...
        with c_v3:
            val_da = scanned['da'] if scanned and scanned.get('da', 0) > 0 else allow_def['da']
            val_hra = scanned['hra'] if scanned and scanned.get('hra', 0) > 0 else allow_def['hra']
            v_da = st.number_input("DA/Mo", value=int(val_da), step=100, key=f"{prefix}_vda")
            v_hra = st.number_input("HRA/Mo", value=int(val_hra), step=100, key=f"{prefix}_vhra")
        with c_v4:
            val_gpf = scanned['gpf'] if scanned and scanned.get('gpf', 0) > 0 else allow_def['gpf_nps']
            val_tds = scanned['tds'] if scanned and scanned.get('tds', 0) > 0 else 0.0
            v_gpf = st.number_input("GPF/NPS /Mo", value=int(val_gpf), step=100, key=f"{prefix}_vgpf")
            v_tds = st.number_input("Monthly TDS (Mar-Jan)", value=int(val_tds), step=500, key=f"{prefix}_vtds")
    ...
    # Generate 11 Months and apply February balancing
    recs_11 = []
    for idx in range(11):
    ...
    full_12, tax_calc = compute_annual_tax_with_regime(recs_11, regime=regime_sel, decl_data=decl_data)
    st.info(f"🎯 **Automated Balancing Result:** Feb Gross ₹{full_12[11]['gross']:,.0f} | **Feb Balanced TDS:** ₹{tax_calc['feb_tds']:,.0f} | **Final Schedule Balance:** ₹{tax_calc['net_balance']:,.0f} (NIL)")
            ...
            if not (pan_in and v_name and ch_office and ch_ddo_id):
                ...
                conn.cursor().execute('''INSERT INTO employee_yearly_records (
                    ...
                    (GLOBAL_AY, ch_ddo_id, pan_in, v_name, v_des, ch_office, status_sel, s_lvl,
                     "NPS" if "NPS" in s_pen else "GPF", regime_sel, s_bsc, s_incm, s_incb, v_da, v_hra, 500.0,
    ...
    with tab_admin:
        ...
        if not st.session_state.admin_logged_in:
            ...
            with adm_sub_tab1:
                ...
                with c_dt1: filter_from = st.date_input("From Date", value=date(2025, 4, 1))
            ...
            # --- 4. STATUTORY TDS DEPOSITS (CHALLAN / BOOK ADJUSTMENT) ---
            with adm_sub_tab4:
                ...
                if emp_list:
                    ...
                    with st.form("tds_deposit_form"):
                        ...
                        with cdp6: dep_bsr = st.text_input("BSR Code / BIN")
                        with cdp7: dep_challan = st.text_input("Challan Serial / DDO Serial No")
