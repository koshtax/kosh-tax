...
def parse_slip_in_memory(uploaded_file):
    extracted = {'basic': 49000.0, 'da': 0.0, 'hra': 0.0, 'med': 500.0, 'gpf': 0.0, 'pan': '', 'name': '', 'designation': '+2 Teacher', 'gpf_no': '', 'account_no': ''}
    try:
        ...
        gpf_m = re.search(r'(?:GPF\s*No\.?|GPF No)\s*[:=-]?\s*([A-Z0-9\/\-]+)', full_text, re.IGNORECASE)
        if gpf_m: extracted['gpf_no'] = gpf_m.group(1).strip()
        
        ac_m = re.search(r'(?:A\/C\s*No\.?|Account No)\s*[:=-]?\s*([0-9]+)', full_text, re.IGNORECASE)
        if ac_m: extracted['account_no'] = ac_m.group(1).strip()
...
def render_full_employee_suite(is_admin_mode=False, prefix="emp"):
    ...
    st.caption("🔒 Zero-Disk In-Memory Slip Scanner: Parsed inside RAM buffer without saving to server storage.")
    ...
    if scanned and slip_up:
        st.success(f"⚡ Extracted: Basic ₹{scanned['basic']:,.0f} | DA ₹{scanned['da']:,.0f} | HRA ₹{scanned['hra']:,.0f}")
    ...
    with st.container(border=True):
        ...
        with c_v1:
            v_name = st.text_input("Full Name *", value=prof[1] if prof else (scanned['name'] if scanned and scanned['name'] else "Employee Name"), key=f"{prefix}_vn")
        ...
        with c_v2:
            ...
            v_eml = st.text_input("Email (Dispatch)", value=prof[4] if prof else "", key=f"{prefix}_ve")
        with c_v3:
        ...
        with c_v4:
            ...
            v_tds = st.number_input("Monthly TDS (Mar-Jan)", value=0, step=500, key=f"{prefix}_vtds")
    ...
    st.info(f"🎯 **Automated Balancing:** Feb Gross ₹{full_12[11]['gross']:,.0f} | **Feb Balanced TDS:** ₹{tax_calc['feb_tds']:,.0f} | **Final Schedule Balance:** ₹{tax_calc['net_balance']:,.0f} (NIL)")
