...
def parse_slip_in_memory(uploaded_file):
    ...
    try:
        ...
        if jul_b and jul_b > mar_b:
            ...
            extracted['inc_month'] = '1st JULY'
            extracted['inc_basic'] = get_next_matrix_cell("Level 7 (GP 4600)", mar_b)

        # 7th CPC Matching & Overlap Check
        matching = get_matching_levels(extracted['basic'])
        extracted['matching_levels'] = matching
        if matching:
            extracted['pay_level'] = matching[0]
        else:
            extracted['pay_level'] = "Level 7 (GP 4600)"
...
def render_full_employee_suite(is_admin_mode=False, prefix="emp"):
    ...
    if slip_up:
        if f"{prefix}_last_uploaded" not in st.session_state or st.session_state[f"{prefix}_last_uploaded"] != slip_up.name:
            ...
            # Govt of Jharkhand auto-lock logic
    ...
    # Scanned metrics review
    if scanned and scanned.get('pan'):
        with st.expander("📋 Extracted Slip Data Summary & 7th CPC Inspection", expanded=True):
    ...
    # 7th CPC Matrix & Increment Level Widgets
    ...
    with c_pen2: 
        s_vgp = st.number_input("Voluntary GPF/Mo", value=int(prof[15] if prof else 0), step=500, key=f"{prefix}_vgp") if "GPF" in s_pen else 0.0
