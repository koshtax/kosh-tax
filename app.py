    ...
    try:
        ...
        for p_txt in pages_text:
            ...
            if is_arrear:
                ...
                m_name = f"{m_match.group(1)} {m_match.group(2)}" if m_match else "BACK-DATED ARREAR"
                extracted['custom_arrear_rows'].append({
                    'month': m_name, 'basic': b_val, 'da': da_val, 'hra': hra_val, 'medical': med_val or 0.0,
                    'gross': b_val + da_val + hra_val + (med_val or 0.0), 'gpf': gpf_val, 'gis': 0.0, 'ptax': 0.0, 'tds': tds_val,
                    'net': (b_val + da_val + hra_val + (med_val or 0.0)) - gpf_val - tds_val
                })
                if "JUL-DEC 2024" in p_up:
                    extracted['arrear_da'] += da_val if da_val > 0 else get_clean_num(p_up, ["TOTAL"])
                else:
                    tot_p = get_clean_num(p_up, ["TOTAL"])
                    extracted['arrear_pay'] += tot_p if tot_p > 0 else b_val
                if tds_val > 0:
...
def compute_annual_tax_with_regime(records_11, regime="NEW REGIME", decl_data=None, manual_arrear_da=0.0, manual_arrear_pay=0.0, extra_tds=0.0, custom_rows=None):
    ...
    # PTax Annual Cap Logic (Max 2500 if gross > 10L, else 2400/2600 based on months count)
    total_working_months = 11 + len(custom_rows) + 1  # 11 regular + custom + feb
...
<!-- SECTION 4: MONTHLY SALARY LEDGER (LANDSCAPE - LARGE COMFORTABLE FONT) -->
<div class="page-landscape">
  ...
  <table class="border" style="font-size: 10px; margin-top: 6px;">
    <tr class="bg-gray center bold">
      <th style="width: 12%; padding: 5px;">माह</th>
      ...
      <th style="width: 9%; padding: 5px;">Arrear</th>
      <th style="width: 10%; padding: 5px;">सकल (Gross)</th>
      ...
      <th style="width: 11%; padding: 5px;">शुद्ध (Net)</th>
    </tr>
    {% for r in records %}
    <tr>
      <td style="padding: 4px;">{{ r.month }}</td>
      ...
      <td class="right" style="padding: 4px;">{{ "%.0f"|format(r.arrear_da + r.arrear_pay) }}</td>
    ...
    <tr class="bold bg-gray">
      ...
      <td class="right" style="padding: 5px;">{{ "%.0f"|format(tax.tot_arrear_da + tax.tot_arrear_pay) }}</td>
      <td class="right" style="padding: 5px;">{{ "%.0f"|format(tax.gross) }}</td>
      <td class="right" style="padding: 5px;">{{ "%.0f"|format(totals.gpf) }}</td>
      <td class="right" style="padding: 5px;">{{ "%.0f"|format(totals.gis) }}</td>
      <td class="right" style="padding: 5px;">{{ "%.0f"|format(tax.ptax) }}</td>
      <td class="right" style="padding: 5px;">{{ "%.0f"|format(tax.tds_paid) }}</td>
...
def generate_pdf_bundle(ddo_dict, emp_dict, full_records, tax_summary, deposits=None, decl_dict=None, is_trial=False):
    ...
    totals['ptax'] = sum(r['ptax'] for r in full_records)
...
def render_full_employee_suite(is_admin_mode=False, prefix="emp"):
    ...
    # DDO Master Section with TAN Lookup & Add/Modify
    ...
    if sel_ddo_dropdown != "--- SELECT OR ENTER NEW TAN ---":
        ...
        with st.expander("📝 Modify Existing DDO Center Details", expanded=False):
            with st.form(f"mod_ddo_form_{ch_ddo_id}"):
                ...
                m_fat = st.text_input("Father's Name (S/O)", value=matched_ddo[5] if len(matched_ddo)>5 and matched_ddo[5] else "", key=f"{prefix}_mod_fat").upper()
                ...
                if st.form_submit_button("Update DDO Details"):
                    conn = sqlite3.connect(DB_NAME)
                    conn.cursor().execute("UPDATE ddo_masters SET officer_name=?, father_name=?, tan=?, address=?, city=?, pincode=? WHERE id=?",
                                          (m_off, m_fat, m_tan, m_add, m_city, m_pin, ch_ddo_id))
                    conn.commit()
                    conn.close()
    ...
    if st.button("📥 Download Free Trial Copy (Watermarked PDF)", use_container_width=True, key=f"{prefix}_trial_btn"):
        if not (pan_in and v_name and ch_office):
            ...
            if ddo_row_preview:
                ddo_d_trial = {
                    'state': ddo_row_preview[1].upper(), 'district': ddo_row_preview[2].upper(), 'department': ddo_row_preview[3].upper(), 
                    'officer_name': ddo_row_preview[4].upper(), 'father_name': ddo_row_preview[5].upper(), 'tan': ddo_row_preview[6].upper(), 
                    'address': ddo_row_preview[7].upper() if ddo_row_preview[7] else '', 'city': ddo_row[8].upper() if ddo_row[8] else ddo_row[2].upper(), 'pincode': ddo_row[9] if ddo_row[9] else ''
                }
            else:
                ddo_d_trial = {
    ...
    if is_admin_mode:
        if st.button("🚀 Save & Generate Official Unlocked PDF Bundle", use_container_width=True, key=f"{prefix}_adm_btn"):
            if not (pan_in and v_name and ch_office and ch_ddo_id and v_mob and v_eml):
                ...
                ddo_d = {
                    'state': ddo_r[1].upper(), 'district': ddo_r[2].upper(), 'department': ddo_r[3].upper(), 
                    'officer_name': ddo_r[4].upper(), 'father_name': ddo_r[5].upper(), 'tan': ddo_r[6].upper(), 
                    'address': ddo_r[7].upper() if ddo_r[7] else '', 'city': ddo_r[8].upper() if ddo_r[8] else ddo_r[2].upper(), 'pincode': ddo_r[9] if ddo_r[9] else ''
                }
    ...
    if st.button("Search Archive & Generate Download"):
        if not rd_pan:
            ...
            if not y_rec:
                ...
                ddo_d = {
                    'state': ddo_row[1].upper(), 'district': ddo_row[2].upper(), 'department': ddo_row[3].upper(), 
                    'officer_name': ddo_row[4].upper(), 'father_name': ddo_row[5].upper(), 'tan': ddo_row[6].upper(), 
                    'address': ddo_row[7].upper() if ddo_row[7] else '', 'city': ddo_row[8].upper() if ddo_row[8] else ddo_row[2].upper(), 'pincode': ddo_row[9] if ddo_row[9] else ''
                }
