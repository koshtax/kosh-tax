...
def parse_slip_in_memory(uploaded_file):
    extracted = {
        'name': '', 'pan': '', 'designation': '', 'gpf_no': '', 'office_name': '',
        'basic': 0.0, 'da': 0.0, 'hra': 0.0, 'medical': 0.0, 'gpf': 0.0, 'gis': 30.0,
        'ptax': 200.0, 'tds': 0.0, 'inc_month': '1ST JULY', 'inc_basic': 0.0,
        'pay_level': '', 'matching_levels': [], 'auto_district': 'KHUNTI',
        'auto_state': 'JHARKHAND', 'employer_type': 'STATE GOVERNMENT',
        'pension_type': 'OLD PENSION (GPF / OPS)', 'custom_arrear_rows': []
    }
    try:
        ...
        if name_m:
            ...
            if len(c) > 2: extracted['name'] = c

        known_designations = ["CLERK", "LIPIK", "ASSISTANT TEACHER", "TEACHER", "HEADMASTER", "PRINCIPAL", "ACCOUNTANT"]
        found_des = ""
        for des in known_designations:
            if des in raw_clean:
                found_des = des
                break
        extracted['designation'] = found_des
        ...
        for p_txt in pages_text:
            ...
            if b_val > 1000:
                ...
                month_range_match = re.search(r'([A-Z]{3,9}\s*[-–TO]+\s*[A-Z]{3,9}\s*\d{4})', p_up)
                if month_range_match or b_val > 40000:
                    period_name = month_range_match.group(1) if month_range_match else "MULTI-MONTH CUMULATIVE SALARY"
                    da_v = get_clean_num(p_up, ["DA"])
                    hra_v = get_clean_num(p_up, ["HRA"])
                    med_v = get_clean_num(p_up, ["MEDICAL"])
                    gpf_v = get_clean_num(p_up, ["GPF", "NPS"])
                    extracted['custom_arrear_rows'].append({
                        'month': f"{period_name} (CUMULATIVE)",
                        'basic': b_val, 'da': da_v or 0.0, 'hra': hra_v or 0.0, 'medical': med_v or 0.0,
                        'arrear_da': 0.0, 'arrear_pay': 0.0,
                        'gross': b_val + (da_v or 0.0) + (hra_v or 0.0) + (med_v or 0.0),
                        'gpf': gpf_v or 0.0, 'gis': 30.0, 'ptax': 200.0, 'tds': 0.0,
                        'net': (b_val + (da_v or 0.0) + (hra_v or 0.0) + (med_v or 0.0)) - (gpf_v or 0.0) - 230.0
                    })

        regular_basics = [b for b in all_basics if b < 40000]
        real_basic = regular_basics[0] if regular_basics else (all_basics[0] if all_basics else 0.0)
        extracted['basic'] = real_basic

        if real_basic > 0:
            determined_level, matching_levels = find_exact_or_nearest_level(real_basic)
            extracted['matching_levels'] = matching_levels
            extracted['pay_level'] = determined_level
            extracted['inc_basic'] = get_next_matrix_cell(determined_level, real_basic)

        if "JANUARY" in raw_clean or "JAN" in raw_clean:
        ...
        for p_txt in pages_text:
            p_up = p_txt.upper()
            if not ("CUMULATIVE" in p_up or "ARREAR" in p_up):
                da_val = get_clean_num(p_up, ["DA"])
                if da_val > 0 and da_val < 30000: extracted['da'] = da_val
                ...
                gpf_val = get_clean_num(p_up, ["GPF", "NPS"])
                if gpf_val > 0 and gpf_val < 15000: extracted['gpf'] = gpf_val
                tds_val = get_clean_num(p_up, ["I.TAX", "ITAX", "TDS"])
                if tds_val > 0: extracted['tds'] = tds_val
                ptax_val = get_clean_num(p_up, ["PTAX", "PROFESSIONAL TAX"])
                if ptax_val > 0: extracted['ptax'] = ptax_val
            ...
            if is_arrear:
                ...
                if m_m and b_val > 0:
                    ...
                    monthly_basics[m_str] = b_val
                       ...
                       # Agar dictionary mein March/April na mile, toh poore text se pehla valid Basic amount utha lo
        found_basic = 0.0
        for p_txt in pages_text:
            b_val = get_clean_num(p_txt.upper(), ["BASIC", "मूल वेतन"])
            if b_val > 1000: # Standard basic pay threshold
                found_basic = b_val
                break

        mar_b = monthly_basics.get("MAR", 0.0) or monthly_basics.get("APR", 0.0) or found_basic or 55200.0
