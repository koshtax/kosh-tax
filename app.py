...
def parse_slip_in_memory(uploaded_file):
    ...
    try:
        ...
        for p_txt in pages_text:
            p_up = p_txt.upper()
            is_arrear = bool(re.search(r'ARREAR', p_up))
...
def render_full_employee_suite(is_admin_mode=False, prefix="emp"):
    ...
    slip_up = st.file_uploader(
        "Upload Salary Slip (PDF) — District, Sector, 7th CPC Matrix & Arrears auto-detect ho jayenge:", 
        type=["pdf"], 
        key=f"{prefix}_slip"
    )
    ...
    with c_d1:
        if matching_ddos:
            ...
            with st.popover("➕ Create DDO Center"):
                in_off = st.text_input("Officer Name", placeholder="e.g. PRINCIPAL / INCHARGE", key=f"{prefix}_in_off").upper()
                in_fat = st.text_input("Father's Name (S/O)", placeholder="e.g. FATHER'S NAME", key=f"{prefix}_in_fat").upper()
                in_tan = st.text_input("TAN Number", placeholder="e.g. PTIK01234A", key=f"{prefix}_in_tan").upper()
                in_add = st.text_input("Office Address", placeholder="e.g. HIGH SCHOOL CAMPUS", key=f"{prefix}_in_add").upper()
                c_c1, c_c2 = st.columns(2)
                with c_c1: in_city = st.text_input("City", placeholder="e.g. KHUNTI", key=f"{prefix}_in_city").upper()
                ...
                if st.button("Save DDO", key=f"{prefix}_in_ddo_btn"):
                    if in_off and in_tan:
                        ...
                        conn.cursor().execute("INSERT INTO ddo_masters (state, district, department, officer_name, father_name, tan, address, city, pincode) VALUES (?,?,?,?,?,?,?,?,?)",
                                              (ch_state.upper(), ch_dist.upper(), ch_dept.upper(), in_off.upper(), in_fat.upper(), in_tan.upper(), in_add.upper(), in_city.upper(), in_pin))
                        conn.commit()
                        conn.close()
                        st.rerun()
                    else:
        ...
        if not (pan_in and v_name and ch_office):
            ...
            if ddo_row_preview:
                ddo_d_trial = {
                    ...
                    'address': ddo_row_preview[7].upper() if ddo_row_preview[7] else '', 'city': ddo_row_preview[8].upper() if ddo_row_preview[8] else ddo_row_preview[2].upper(), 'pincode': ddo_row_preview[9] if ddo_row_preview[9] else ''
                }
            else:
                ddo_d_trial = {
            ...
            if not (pan_in and v_name and ch_office and ch_ddo_id):
                ...
                conn.cursor().execute('''INSERT INTO employee_master_profiles (
                    ...
                    ON CONFLICT(pan) DO UPDATE SET
                        name=excluded.name, last_office=excluded.last_office, last_basic_pay=excluded.last_basic_pay, gpf_no=excluded.gpf_no, employer_type=excluded.employer_type''',
                ...
                conn.cursor().execute('''INSERT INTO employee_yearly_records (
                    ...
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                    ON CONFLICT(ay, pan) DO UPDATE SET payment_status='APPROVED', tax_regime=excluded.tax_regime, employer_type=excluded.employer_type''',
        ...
        if btn_t:
            if not (pan_in and v_name and v_mob and ch_office and ch_ddo_id):
                ...
                conn.cursor().execute('''INSERT INTO employee_yearly_records (
                    ...
                    email, payment_status, payment_mode, utr_no, amount_paid, created_at, created_date)
                    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                    ON CONFLICT(ay, pan) DO UPDATE SET payment_status=excluded.payment_status, tax_regime=excluded.tax_regime, employer_type=excluded.employer_type''',
                    (GLOBAL_AY, ch_ddo_id, pan_in.upper(), v_name.upper(), v_des.upper(), ch_office.upper(), status_sel.upper(), s_lvl.upper(),
                     s_empt.upper(), "GPF" if "GPF" in s_pen else "NPS", regime_sel.upper(), s_bsc, s_incm.upper(), s_incb, v_da, v_hra, 1000.0,
