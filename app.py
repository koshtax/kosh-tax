...
# ============================================================
# KOSH-TAX — GRANULAR SALARY / ARREAR / TDS / FORM-16 PORTAL
# ============================================================
# Design goals:
# - Every salary/arrear bill is an independent transaction.
# - No delete-all/reinsert save workflow.
# - Removed rows become VOID and remain in the audit trail.
# - Validation occurs before database commit.
# - ACTIVE rows are the only rows used by ledger/tax/PDF.
# - Tax/Form-16/Schedule/PDF are blocked on critical reconciliation errors.
# - FY/AY rules are configuration-driven.
# ============================================================
...
APP_VERSION = "2.0.0-granular-audit"
...
# Existing 7th CPC reference matrix retained.
CPC_7TH_MATRIX = {
...
# IMPORTANT: These are application configuration defaults, not a statement
# that they apply to every future year. Change them in Config when rules change.
DEFAULT_TAX_RULES = {
    "AY 2025-26 (FY 2024-25)": {
        ...
        "new_slabs": [
            ...
            [1500000, 999999999999, 0.30],
        ],
        "old_slabs": [
            [0, 250000, 0.00],
            [250000, 500000, 0.05],
            [500000, 1000000, 0.20],
            [1000000, 999999999999, 0.30],
        ],
    },
    "AY 2026-27 (FY 2025-26)": {
...
def init_db():
    ...
    try:
        ...
        # Legacy table migration is intentionally conservative.
        # If old data contains arrears without original periods, mark NEEDS_REVIEW.
...
def reconcile_tds(active_transactions, deposit_rows, strict_quarters=True):
    tx_total = round(sum(money(t.get("tds",0)) for t in active_transactions), 2)

    quarter_keys = ["Q1", "Q2", "Q3", "Q4"]
    quarter_amounts = {q: 0.0 for q in quarter_keys}
    for row in deposit_rows:
        q = str(row["quarter"] or "").upper().strip()
        q = q.split()[0] if q else ""
        if q in quarter_amounts:
            quarter_amounts[q] += money(row["amount"])

    missing = [q for q in quarter_keys if abs(quarter_amounts[q]) < 0.005]
    quarter_total = round(sum(quarter_amounts.values()), 2)

    errors = []
    # If annual employee TDS is zero, no quarter deposit is required.
    # If annual TDS is non-zero, all four quarter records must be explicitly present.
    if strict_quarters and tx_total > 0.01 and missing:
        errors.append("Missing actual TDS deposit records for: " + ", ".join(missing))
    if abs(tx_total - quarter_total) > 0.01:
        errors.append(
            f"TDS reconciliation mismatch: transaction TDS ₹{tx_total:,.2f} "
            f"vs quarter deposit total ₹{quarter_total:,.2f}."
        )
...
def validate_final_outputs(totals, tax, tds_rec):
    errors = list(tds_rec["errors"])
    checks = {
        "gross": abs(money(tax["gross"]) - money(totals["gross"])) <= 0.01,
        "tds": abs(money(tax["tds_paid"]) - money(totals["tds"])) <= 0.01,
        "balance": abs(money(tax["net_balance"]) - (money(tax["total_tax"]) - money(tax["tds_paid"]))) <= 0.01,
        "schedule_gross": abs(money(totals["gross"]) - money(tax["gross"])) <= 0.01,
        "schedule_taxable": money(tax["taxable_income"]) >= 0,
    }
    for name, ok in checks.items():
        if not ok:
            errors.append(f"Final reconciliation failed: {name}.")
    return errors
...
def save_transactions(yid, edited_df, fy_rules):
    ...
    try:
        ...
        # Removed UI rows are VOID, never physically deleted.
...
<div class="note">This schedule is generated only after the application's transaction,
tax and TDS reconciliation gates pass.</div>
</div>
...
def render_employee():
    ...
    # Editable fields; identity fields are protected by being disabled.
    display_cols = [
    ...
    if st.button("💾 Save Ledger Transactions & Validate", type="primary"):
        ...
        if not ok:
            st.error("❌ SAVE BLOCKED — NO PARTIAL DATABASE COMMIT WAS MADE.")
            for e in errors:
                st.write("•", e)
            return
        ...
        # Re-read authoritative ACTIVE state after commit.
        active = load_transactions(yid)
        errors, warnings2 = validate_transactions(active)
        if errors:
            st.error("❌ POST-COMMIT AUTHORITATIVE VALIDATION FAILED — FINALIZATION BLOCKED.")
            for e in errors:
                st.write("•", e)
            return
        ...
        final_errors = validate_final_outputs(totals, tax, tds_rec)

        if final_errors:
            st.error("🔒 FINALIZATION BLOCKED — TAX / FORM-16 / SCHEDULE / PDF WAS NOT GENERATED.")
            for e in final_errors:
                st.write("•", e)
            return

        st.success("✅ All validation and reconciliation gates passed.")
    ...
    if active:
        ...
        st.json({
            ...
            "Quarter TDS (recorded)": tds_rec["quarters"],
        })


# --------------------------- TDS / declaration / DDO admin ---------------------------
