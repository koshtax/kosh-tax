...
def parse_slip_in_memory(uploaded_file):
    ...
    try:
        ...
        if jul_b and jul_b > mar_b:
            ...
            extracted['inc_month'] = '1st JULY'
            extracted['inc_basic'] = get_next_matrix_cell("Level 7", mar_b)

        for lvl, cells in PAY_MATRIX.items():
            if extracted['basic'] in cells or extracted['inc_basic'] in cells:
                extracted['pay_level'] = lvl
                break
...
def render_full_employee_suite(is_admin_mode=False, prefix="emp"):
    ...
    slip_up = st.file_uploader(
        "Upload Salary Slip (PDF) — PAN, Name, Basic, Level, Increment Month aur Arrears auto-extract ho jayenge[span_3](start_span)[span_3](end_span)[span_4](start_span)[span_4](end_span):", 
        type=["pdf"], 
        key=f"{prefix}_slip"
    )
    ...
    if slip_up:
        if f"{prefix}_last_uploaded" not in st.session_state or st.session_state[f"{prefix}_last_uploaded"] != slip_up.name:
            ...
            # Dynamic session state overwrite taaki screen ke dabbe instantly update ho jayein
            if scanned.get('pan'):
    ...
    # Scanned metrics card
    if scanned and scanned.get('pan'):
        with st.expander("📋 Extracted Slip Data Summary & Arrear Breakdown", expanded=True):
            r1, r2, r3, r4 = st.columns(4)
            r1.metric("Employee Name", scanned.get('name') or "N/A")
            r2.metric("PAN Number", scanned.get('pan') or "N/A")
            r3.metric("Pay Level", scanned.get('pay_level') or "Level 7")
            r4.metric("Increment Month", scanned.get('inc_month') or "1st JULY")

            r5, r6, r7, r8 = st.columns(4)
            r5.metric("March Basic", f"₹{scanned.get('basic', 0):,.0f}")
            r6.metric("DA Arrear", f"₹{scanned.get('arrear_da', 0):,.0f}")
            r7.metric("Pay Arrear", f"₹{scanned.get('arrear_pay', 0):,.0f}")
            r8.metric("Monthly TDS", f"₹{scanned.get('tds', 0):,.0f}")
    ...
    # Institutional Location Hierarchy
    ...
    # Pay Matrix & Increment Month widgets
    c_m1, c_m2, c_m3, c_m4 = st.columns(4)
    with c_m1: s_lvl = st.selectbox("Pay Level", list(PAY_MATRIX.keys()), key=f"{prefix}_lvl")
    ...
    with c_m4: s_incb = st.number_input("Incremented Basic", step=100, key=f"{prefix}_incb")

    c_pen1, c_pen2 = st.columns(2)
    with c_pen1: s_pen = st.radio("Pension Scheme", ["Old Pension (GPF)", "NPS (10% Basic+DA)"], index=0, horizontal=True, key=f"{prefix}_pen")
    with c_pen2: s_vgp = st.number_input("Voluntary GPF/Mo", value=int(prof[16] if prof else 0), step=500, key=f"{prefix}_vgp") if "GPF" in s_pen else 0.0
    ...
    st.info(f"🎯 **Automated Balancing Summary:** Gross: ₹{tax_calc['gross']:,.0f} | DA Arrear: ₹{tax_calc['tot_arrear_da']:,.0f} | Pay Arrear: ₹{tax_calc['tot_arrear_pay']:,.0f} | **Feb Balancing TDS:** ₹{tax_calc['feb_tds']:,.0f} | **Final Schedule Balance:** ₹{tax_calc['net_balance']:,.0f} (NIL)[span_5](start_span)[span_5](end_span)")
            ...
            if not (pan_in and v_name and ch_office and ch_ddo_id):
                ...
                conn.cursor().execute('''INSERT INTO employee_master_profiles (
                    ...
                    (pan_in, v_name, v_des, v_mob, v_eml, ch_state, ch_dist, ch_dept, ch_office, ch_ddo_id,
                     s_lvl, s_bsc, s_incm, "GPF" if "GPF" in s_pen else "NPS", s_vgp, now_str))
