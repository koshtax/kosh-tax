...
def init_db():
    ...
    # Ensure updated UPI default if already present
    c.execute("UPDATE app_settings SET value='nitinmallick111-1@okicici' WHERE key='upi_id' AND value='mallick.nitin@okaxis'")
...
def generate_pdf_bundle(ddo_dict, emp_dict, full_records, tax_summary, deposits=None, decl_dict=None):
    ...
    rendered = Template(HTML_MASTER_TEMPLATE).render(
        ...
        totals=totals, deposits=deposits or [], decl=decl_dict, today_date=date.today().strftime("%d.%m.%Y")
    )
...
def render_full_employee_suite(is_admin_mode=False, prefix="emp"):
    ...
    with st.container(border=True):
        ...
        with c_v3:
            v_gpf_no = st.text_input("GPF / PRAN Number (Employee Ref No)", key=f"{prefix}_vgpf_no")
